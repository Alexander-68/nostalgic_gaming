...
- **No dedicated ERASE control** (Sudoku): mistakes are corrected via UNDO rather than a separate erase button/key, keeping the touch chrome (and this doc) simpler — one fewer button to lay out per aspect ratio.
